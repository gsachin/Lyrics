...
class WebServiceCall : NSObject {
    ...
    public class var shared:WebServiceCall {
        struct Singleton {
            static let instance = WebServiceCall()
        }
        return Singleton.instance
    }
    
    public class func sharedInstance()->WebServiceCall {
        return WebServiceCall.shared
    }
            ...
            let task = session.dataTask(with: url! as URL, completionHandler: {data, response, error -> Void in
                ...
                do
                {
                    ...
                    success(json)
                        ...
                        if (jsonComponents?.count)! >= 2 {
                            let josnString = jsonComponents?[1]//.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)
                           let json = CommonMethods.sharedInstance.convertToDictionary(text:josnString!)
